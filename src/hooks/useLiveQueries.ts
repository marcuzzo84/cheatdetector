...
export const useLiveKPIs = () => {
  ...
  const fetchKPIs = async () => {
    try {
      ...
      // Set a timeout for the query - increased from 30 seconds to 90 seconds
  ...
  const calculateFallbackKPIs = async (): Promise<KPIData> => {
    try {
      ...
      const timeoutPromise = new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Fallback query timeout')), 45000)
      );
  ...
  useEffect(() => {
    // Initial fetch with timeout - increased from 45 seconds to 100 seconds
...
export const useLiveSuspicionTrends = () => {
  ...
  const calculateFallbackTrends = async (): Promise<SuspicionTrend[]> => {
    try {
      ...
      const timeoutPromise = new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Fallback trends timeout')), 15000)
      );
  ...
  useEffect(() => {
    // Initial fetch with timeout - increased from 70 seconds to 130 seconds
    const fetchTimeout = setTimeout(() => {
