/*
  # Complete SECURITY INVOKER Implementation with RLS Policies

  1. Security Changes
    - Enable RLS on all relevant tables
    - Create comprehensive RLS policies for all tables
    - Convert all views to use SECURITY INVOKER
    - Create helper functions with SECURITY INVOKER

  2. Tables with RLS
    - scheduler_jobs, scheduler_metrics, sync_cursor
    - scores, games, players, player_files

  3. Views Converted
    - v_scheduler_dashboard, v_import_stats, v_daily_suspicion
    - v_player_file_statistics (ensuring SECURITY INVOKER)

  4. Helper Functions
    - get_dashboard_kpis(), get_suspicion_trends()
    - get_recent_high_risk_games(), get_daily_suspicion_realtime()
*/
...
DROP FUNCTION IF EXISTS public.get_suspicion_trends(integer) CASCADE;
DROP FUNCTION IF EXISTS public.get_recent_high_risk_games(integer) CASCADE;
DROP FUNCTION IF EXISTS public.get_daily_suspicion_realtime() CASCADE;
DROP FUNCTION IF EXISTS public.get_user_storage_usage() CASCADE;
DROP FUNCTION IF EXISTS public.get_file_statistics_summary() CASCADE;
...
CREATE FUNCTION public.get_suspicion_trends(days_back integer DEFAULT 30)
RETURNS TABLE (
    date text,
...
CREATE FUNCTION public.get_recent_high_risk_games(game_limit integer DEFAULT 20)
RETURNS TABLE (
    id uuid,
    game_id uuid,
...
CREATE FUNCTION public.get_daily_suspicion_realtime()
RETURNS TABLE (
    bucket timestamptz,
    rate numeric,
    volume bigint
...
-- Function to get user storage usage (respects RLS)
CREATE FUNCTION public.get_user_storage_usage()
RETURNS TABLE (
    total_files bigint,
    total_size numeric,
    pgn_files bigint,
...
-- Function to get file statistics summary (respects RLS)
CREATE FUNCTION public.get_file_statistics_summary()
RETURNS TABLE (
    total_users bigint,
    total_files bigint,
    total_size numeric,
...
AS $$
    SELECT 
        ...
        CASE 
            WHEN COUNT(DISTINCT user_id) > 0 
            THEN COALESCE(SUM(file_size), 0)::numeric / COUNT(DISTINCT user_id)
            ELSE 0::numeric
...
GRANT EXECUTE ON FUNCTION public.get_file_statistics_summary() TO service_role;

-- Add comment
COMMENT ON FUNCTION public.get_file_statistics_summary() IS 'Get file statistics summary - respects RLS policies';

-- Function to verify admin access
CREATE FUNCTION public.verify_admin_access()
RETURNS boolean
LANGUAGE sql
SECURITY INVOKER
STABLE
...
-- Add comment
COMMENT ON FUNCTION public.verify_admin_access() IS 'Verify if current user has admin access';
