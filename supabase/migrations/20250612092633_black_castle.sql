/*
  # Scheduler Support Migration

  1. Database Functions
    - execute_maintenance_query() for safe database maintenance
    - get_table_statistics() for database health metrics
    - execute_query() for dynamic query execution
    - record_scheduler_metric() for metrics recording

  2. Scheduler Tables
    - scheduler_jobs: Track job executions
    - scheduler_metrics: Store performance metrics

  3. Views and Functions
    - v_scheduler_dashboard: Aggregated dashboard view
    - get_scheduler_status() for API access
...
BEGIN
  RETURN QUERY
  SELECT 
    (pst.schemaname || '.' || pst.relname)::text as table_name,
    pst.n_tup_ins + pst.n_tup_upd - pst.n_tup_del as row_count,
    pg_size_pretty(pg_total_relation_size(pst.schemaname||'.'||pst.relname))::text as table_size,
    pg_size_pretty(pg_indexes_size(pst.schemaname||'.'||pst.relname))::text as index_size,
    pg_size_pretty(pg_total_relation_size(pst.schemaname||'.'||pst.relname) + pg_indexes_size(pst.schemaname||'.'||pst.relname))::text as total_size,
    pst.last_vacuum,
    pst.last_analyze
  FROM pg_stat_user_tables pst
  WHERE pst.schemaname = 'public'
    AND pst.relname IN ('players', 'games', 'scores', 'sync_cursor')
...
-- Create policies for scheduler_jobs
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Create policies for scheduler_metrics
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
...
SELECT 
  ...
  (SELECT COALESCE(SUM(games_imported), 0) FROM scheduler_jobs WHERE started_at > now() - interval '24 hours') as games_imported_24h,
