/*
  # Database Maintenance and Scheduler Functions

  1. Maintenance Functions
    - `execute_maintenance_query` - Safely execute maintenance operations
    - `get_table_statistics` - Get database table statistics
    - `execute_query` - Execute dynamic SELECT queries for scheduler targets
  
  2. Scheduler Tables
    - `scheduler_jobs` - Track execution of scheduled import jobs
    - `scheduler_metrics` - Store scheduler performance metrics
  
  3. Scheduler Functions
    - `record_scheduler_metric` - Record performance metrics
    - `get_scheduler_status` - Get dashboard status
    - `notify_scheduler_event` - Send notifications for job events
  
  4. Views
    - `v_scheduler_dashboard` - Dashboard view for scheduler status
*/
...
-- 4. Create scheduler jobs tracking table if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'scheduler_jobs') THEN
    ...
    -- Create indexes
    CREATE INDEX idx_scheduler_jobs_name_status ON scheduler_jobs(job_name, status);
    CREATE INDEX idx_scheduler_jobs_started_at ON scheduler_jobs(started_at DESC);
  END IF;
END $$;

-- 5. Create scheduler metrics table if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'scheduler_metrics') THEN
    ...
    -- Create indexes
    CREATE INDEX idx_scheduler_metrics_name_timestamp ON scheduler_metrics(metric_name, timestamp DESC);
    CREATE INDEX idx_scheduler_metrics_timestamp ON scheduler_metrics(timestamp DESC);
  END IF;
END $$;
...
DROP VIEW IF EXISTS v_scheduler_dashboard;
CREATE VIEW v_scheduler_dashboard AS
SELECT 
  -- Job statistics
...
-- Create trigger for scheduler notifications
