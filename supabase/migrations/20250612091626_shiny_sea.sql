...
BEGIN
  ...
  DROP POLICY IF EXISTS "Service role can manage sync cursors" ON sync_cursor;
  DROP POLICY IF EXISTS "Authenticated users can read sync cursors" ON sync_cursor;
...
DROP TRIGGER IF EXISTS update_sync_cursor_updated_at ON sync_cursor;
...
-- Grant permissions (revoke first to avoid conflicts)
REVOKE ALL ON FUNCTION get_import_status(text, text) FROM authenticated, service_role;
...
-- Grant permissions (revoke first to avoid conflicts)
REVOKE ALL ON FUNCTION update_sync_cursor(text, text, timestamptz, text, integer) FROM service_role;
...
-- Grant permissions (revoke first to avoid conflicts)
REVOKE ALL ON FUNCTION game_exists(text, text) FROM service_role;
...
DROP VIEW IF EXISTS v_import_stats;
CREATE VIEW v_import_stats AS
SELECT 
  sc.site,
...
-- Grant permissions on view (revoke first to avoid conflicts)
REVOKE ALL ON v_import_stats FROM authenticated, service_role;
...
-- Grant permissions (revoke first to avoid conflicts)
REVOKE ALL ON FUNCTION cleanup_old_sync_cursors(integer) FROM service_role;
GRANT EXECUTE ON FUNCTION cleanup_old_sync_cursors(integer) TO service_role;

-- 9. Create notification function for import completion
...
-- Create trigger for import notifications (drop first if exists)
