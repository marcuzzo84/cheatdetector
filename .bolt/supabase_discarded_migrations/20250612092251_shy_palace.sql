/*
  ...
  1. Database Functions
    - execute_maintenance_query: Execute maintenance SQL queries
    - get_table_statistics: Get database table statistics
    - execute_query: Execute dynamic queries for scheduler targets

  2. Scheduler Tables
    - scheduler_jobs: Track scheduled job executions
    - scheduler_metrics: Store scheduler metrics

  3. Security
    - Service role access for maintenance functions
    - Proper RLS policies
*/
...
AS $$
  ...
  WHERE schemaname = 'public'
    AND tablename IN ('players', 'games', 'scores', 'sync_cursor')
