/*
  # Scheduler and Database Maintenance Functions

  1. Database Maintenance
    - `execute_maintenance_query()` function for VACUUM, ANALYZE, REINDEX
    - `get_table_statistics()` function for table stats
    - `execute_query()` function for dynamic SELECT queries

  2. Scheduler Infrastructure
    - `scheduler_jobs` table for tracking job executions
    - `scheduler_metrics` table for performance metrics
    - `v_scheduler_dashboard` view for dashboard data

  3. Functions and Triggers
    - `record_scheduler_metric()` for metrics collection
    - `get_scheduler_status()` for dashboard API
    - `notify_scheduler_event()` trigger for job notifications

  4. Security
    - RLS enabled on all tables
    - Service role and authenticated user policies
    - Restricted function permissions
*/
